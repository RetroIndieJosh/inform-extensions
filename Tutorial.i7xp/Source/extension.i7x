...
After saying hello to the old gentleman:
...
After quizzing the old gentleman about their name:
	...
	introduce  the old gentleman.

After examining the old gentleman for the first time, tutsay "He took the apple! Well, that's okay. You didn't need it anyway. But that crowbar might be helpful for getting the window open. Maybe ask him for it?"

Instead of requesting the old gentleman for the apple:
...
Instead of requesting the old gentleman for the crowbar:
	if the current interlocutor is not the old gentleman, try saying hello to the old gentleman;
...
After informing the old gentleman about the office window:
...
After going down from the tree branch when the apple is in the bottom of the tree:
	now the old gentleman is in the bottom of the tree;
	now the old gentleman carries the apple;
	continue the action.
...
Example: * Starting the tutorial when play begins.
